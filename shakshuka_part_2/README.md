...
Download this [skeleton](https://assets.aaonline.io/fullstack/html-css/micro-projects/display_box_model/skeleton.zip).

The purpose of this section is to learn the box model and the differences between the three primary display properties (block, inline, inline-block). The anchor tags from step 2 are naturally inline elements, which is why they act oddly when provided padding. Spacing with margin and padding is vital to mastering CSS design. We will also use the `inline-block` property to get practice positioning elements to create a desired layout.

Update the css file as follows to practice styling with different display and spacing properties.
...
When you're completed, compare your [solution](https://assets.aaonline.io/fullstack/html-css/micro-projects/display_box_model/solution.zip) to ours.

### CSS Reset

Download this [skeleton](https://assets.aaonline.io/fullstack/html-css/micro-projects/css_reset/skeleton.zip). Work out of `css_reset.css`
...
-   Replace the fixed pixel widths of our `aside` and `main` content section with a fluid grid system similar to those seen in the [CSS Grid System video](https://open.appacademy.io/learn/full-stack-online/sql/shakshuka-part-2). Create a css file called`custom_grid.css` and add these changes to that file. Note that this will require updating `index.html` with respective container class names.
...
Download the [solution](https://assets.aaonline.io/fullstack/html-css/micro-projects/flex/solution.zip) to compare your work to ours.